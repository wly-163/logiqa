...
def _doc(doc_id: str, name: str, text: str, *, tags: str = "AGV", section: str = "AGV运行限值"):
...
def test_detects_explainable_negation_and_threshold_conflicts():
    left = _doc(
        "d1",
        "AGV运行规程V1.pdf",
        "AGV冷机运行时必须投入。AGV库区温度不得超过80℃。",
    )
    right = _doc(
        "d2",
        "AGV运行规程V2.pdf",
        "AGV冷机运行时严禁投入。AGV库区温度不得超过85℃。",
    )
...
def test_different_equipment_or_topic_is_not_compared():
    breaker = _doc(
        "d1", "叉车规程.pdf", "叉车急停时间必须符合出厂要求。",
        tags="叉车", section="叉车检修",
    )
    battery = _doc(
...
def test_superseded_document_is_excluded_from_conflict_scan():
    ...
    left = _doc("d1", "旧规程.pdf", "AGV冷机运行时必须投入。")
    left.metadata = _metadata(now, version_status="superseded")
    right = _doc("d2", "新规程.pdf", "AGV冷机运行时严禁投入。")
