...
def test_detect_injection_clean_technical_query():
    ...
    flagged, _ = safety.detect_injection("自动导引车温度异常如何处置")
    assert not flagged
...
def test_extract_high_risk_keywords(monkeypatch):
    from app.config import settings
    monkeypatch.setattr(
        settings,
        "HIGH_RISK_KEYWORDS",
        "危险品,危化品,冷链断链,高空,登高,叉车,AGV,混装,无单出库,口头指令,超载,带电作业",
    )
    risks = safety.extract_high_risk("作业前需排查危险品混装，并防止冷链断链")
    assert "危险品" in risks
    assert "冷链断链" in risks


def test_extract_high_risk_none():
    assert safety.extract_high_risk("检查库位库温是否正常") == []
