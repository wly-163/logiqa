...
def test_gen_telemetry_returns_correct_fields():
    ...
    assert data["deviceId"] == "agv_01"
    assert data["deviceName"] == "AGV-01"
    assert data["status"] == "running"
    assert 18.0 <= data["temperature"] <= 32.0
