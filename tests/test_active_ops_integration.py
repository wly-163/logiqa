...
async def test_restarted_worker_takes_over_running_proactive_run(
    ...
    async def fake_run_agent(db, persona, prompt, model_type, ctx):
        ...
        assert "禁止执行自动分拣指令下发" in prompt
        assert "AGV 调度控制" in prompt
        return SimpleNamespace(
            answer={
                "summary": "检查温控系统并持续监测",
                "diagnosis": "库温异常",
...
def test_qa_cache_keys_are_tenant_scoped():
    assert qa_service._cache_key("deepseek", "库温高", "tenant-a") != qa_service._cache_key(
        "deepseek", "库温高", "tenant-b"
    )
    assert QaCache.build_hash("deepseek", "库温高", "tenant-a") != QaCache.build_hash(
        "deepseek", "库温高", "tenant-b"
    )
...
async def test_proactive_ticket_source_ref_is_unique_and_idempotent(active_ops_db):
    ...
    assert duplicate["task"] == "核验AGV温控系统"
    assert count == 1
