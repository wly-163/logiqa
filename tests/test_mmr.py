...
def test_tokens_filters_single_char_and_keeps_terms():
    # 单字/标点过滤，术语保留
    assert "的" not in _tokens("自动导引车的温度")
    assert "温度" in _tokens("自动导引车温度异常")


def test_mmr_returns_all_when_below_topk():
    cands = [{"text": "AGV温度异常", "score": 0.9}, {"text": "叉车急停", "score": 0.8}]
...
def test_mmr_prefers_diverse_over_redundant():
    ...
    cands = [
        {"text": "AGV温度异常处置流程步骤", "score": 0.95, "id": 1},
        {"text": "AGV温度异常处置流程", "score": 0.93, "id": 2},
        {"text": "冷链断链应急处理方法", "score": 0.90, "id": 3},
    ]
...
def test_mmr_lambda_near_one_is_pure_relevance():
    ...
    cands = [
        {"text": "AGV温度异常", "score": 0.95, "id": 1},
        {"text": "AGV温度升高", "score": 0.94, "id": 2},
        {"text": "叉车急停", "score": 0.90, "id": 3},
    ]
