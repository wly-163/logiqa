...
def test_search_regulation_wraps_mixed_search(monkeypatch):
    ...
    async def fake_mixed(db, q, topk, model_type=None, tenant=None):
        seen["tenant"] = tenant
        return [{"docName": "手册A", "chunk": "库温超过95度应..."}]
    ...
    out = asyncio.run(agent_tools._t_search_regulation(
        db=None, model_type=None, query="库温高", tenant="tenant-a",
    ))
...
def test_query_equipment_graph_empty_returns_hint(monkeypatch):
    ...
    out = asyncio.run(agent_tools._t_query_equipment_graph(
        marker_db, None, entity="1号AGV", tenant="tenant-a",
    ))
    assert "无" in out
...
def test_diagnose_persona_config():
    ...
    assert "供应链" in p.system_prompt or "诊断" in p.system_prompt or "物流" in p.system_prompt
...
def test_diagnose_fallback_strips_prefix_and_calls_domain(monkeypatch):
    ...
    out = asyncio.run(agent_personas._diagnose_fallback(
        db=None, user_msg="异常症状：华东RDC爆仓", model_type=None))
    assert captured["symptom"] == "华东RDC爆仓"  # 前缀已剥离
...
def test_diagnose_agent_migration_returns_stable_schema(monkeypatch):
    ...
    async def fake_mixed(db, q, topk, model_type=None):
        return [{"docName": "规程A", "chunk": "库温高处置..."}]
    ...
    async def fake_ticket(db, task, mt, topk):
        return {"ticket": {"device": "1号AGV", "steps": ["停机"], "safety": [], "risks": []}}
    monkeypatch.setattr(agent_tools.retrieval_service, "mixed_search", fake_mixed)
    monkeypatch.setattr(agent_tools.kg_service, "graph_context", fake_graph)
    monkeypatch.setattr(agent_tools.domain_service, "similar_case", fake_case)
    monkeypatch.setattr(agent_tools.domain_service, "generate_ticket", fake_ticket)
    ...
    fake = FakeProvider([
        {"content": "查规程", "tool_calls": [
            {"id": "1", "name": "search_regulation", "arguments": {"query": "库温高"}}]},
        {"content": "查图谱", "tool_calls": [
            {"id": "2", "name": "query_equipment_graph", "arguments": {"entity": "1号AGV"}}]},
...
def test_log_tool_call_writes_record(monkeypatch):
    ...
    asyncio.run(agent_tool_audit_service.log_tool_call(
        ...
        args={"query": "库温高"}, result="证据摘要", error=False,
    ...
    assert rec.error is False and "库温高" in rec.args_json
...
def test_qa_persona_config():
    ...
    assert "draft_ticket" not in QA_PERSONA.allowed_tools  # 问答不需要作业单
...
def test_alert_persona_config():
    ...
    assert "draft_ticket" in ALERT_PERSONA.allowed_tools  # 告警处置可生成作业单
