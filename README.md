...
flowchart TB
    ...
    L4["🤖 模型层 · Provider(三家可切换)<br/>LLM: 三路云模型 · Embedding: 云1024 + 本地bge512<br/>Rerank: gte-rerank-v2 · VLM: 多模态视觉模型 · Agent 引擎"]:::l4
...
| **operator** | 一线仓配 | 问答 + 诊断 + 作业单 + 反馈(只读文档) |
...
flowchart TB
    ...
    subgraph BIZ["业务域"]
        ...
        DOM["🩺 领域诊断<br/>异常诊断Agent/相似案例/作业单/审核"]:::operator
...
| **Diagnose** | `/diagnose` | 诊断 Agent · 相似案例 · 作业单生成/审核 |
...
flowchart TD
    ...
    DT -- "故障口语/同义词<br/>(如 爆仓/断链)" --> DN["dense<br/>仅向量召回"]:::out
...
flowchart LR
    ...
    subgraph ST["多轮指代消解(仅多轮)"]
        STQ["rewrite_standalone<br/>standalone_query.py:12<br/>'它的处置呢'→'冷链断链的处置步骤'"]:::multi
    end
...
flowchart TD
    ...
    subgraph DENSE["稠密双路(asyncio.gather 并行)·_dense_and_sparse :114"]
        ...
        CL --> MC1[("Milvus logiqa_chunks<br/>HNSW search<br/>milvus_client.py:63")]:::store
        BG --> MC2[("Milvus logiqa_chunks_bge<br/>HNSW search")]:::store
    end
...
> RRF 把多路召回等权融合;Rerank 用**云端推理接口 `gte-rerank-v2`**(云 HTTP 原生 API,**非本地 bge-reranker**)做交叉注意力精排,失败降级回退 RRF。
...
**图谱写入**:`extract_triples`(`kg_service.py:162`)LLm 抽三元组(6 块/批),schema 约束 13 关系白名单 + 实体归一(`一盘货/统仓统配→一盘货统仓统配`)+ MERGE 幂等双写 MySQL + Neo4j。节点 `:Entity{name,type}`(Node/Exception/Action),统一有向关系 `:REL{type,doc_id}`。
...
**事件来源**:`meta`(`qa_service.py:787`)、`token`(`:803`)、`tool_step`(`:542`,仅 agent)、`done`(末尾)。token 来自 `get_llm_provider(model_type).stream()`(`:801`),三家 provider 透传 兼容协议 SDK `stream=True`。WebSocket 增强版 `/qa/answer/ws`(`qa.py:304`,双向 `?token=JWT` 鉴权)。
...
flowchart LR
    ...
    H3 --> P3["safety_banner :74<br/>含'危险品/冷链/叉车/AGV/高空'等<br/>→ 追加⚠安全提示<br/>(已有则不重复)"]:::plugin
    P3 --> OUT([返回答案]):::main
...
flowchart TD
    ...
    TR --> TOOLS["DEFAULT_REGISTRY<br/>agent_tools.py<br/>检索/图谱/案例/作业单工具"]:::tool
...
flowchart LR
    ...
    MILVUS[("Milvus 2.4 :19530<br/>双 collection 向量<br/>logiqa_chunks(云1024d)<br/>logiqa_chunks_bge(bge512d)<br/>HNSW M=16/efC=200/COSINE")]:::milvus
...
| LLM(云,可切换) | 云模型 1 `deepseek-chat` / 云接口 `qwen-plus` / 云端模型(endpoint_id) |
| Embedding(云) | 云接口 `text-embedding-v3`(1024维)/ 云端模型 |
| Embedding(本地) | `bge-small-zh-v1.5`(512维)· sentence-transformers |
| Rerank | **云接口 `gte-rerank-v2`**(云端原生 HTTP) |
...
│   │   └── data/{logistics_terms.json,golden_qa.json}
│   ├── migrations/               # Alembic
│   ├── Dockerfile
│   └── requirements.txt
...
| `MILVUS_COLLECTION` / `MILVUS_COLLECTION_BGE` | 双 collection | logiqa_chunks / logiqa_chunks_bge |
...
| 方法 | 路径 | 说明 |
|---|---|---|
| POST | `/api/domain/diagnose` `/diagnose-agent` `/diagnose-debate` | 异常诊断(Agent/多Agent辩论) |
| POST | `/api/domain/similar-case` | 相似历史案例 |
| POST | `/api/domain/ticket` `/ticket/audit` | 作业单生成/审核 |
...
# 接收方:
tar -xzf logiqa-release-*.tar.gz && cd logiqa-release-*
cp .env.template .env && vim .env     # 填 API Key
./install.sh up                       # 一键(含 logiqa.sql 自动导入)
```
...
**Q: Rerank 用的是什么?**
A: 云端推理接口 `gte-rerank-v2`(原生 HTTP API),非本地 bge-reranker。失败降级回退 RRF,`DEGRADED{tag=rerank}` 可观测。
...
见 [CONTRIBUTING.md](CONTRIBUTING.md)。行为准则见 [CODE_OF_CONDUCT.md](CODE_OF_CONDUCT.md)。安全披露见 [SECURITY.md](SECURITY.md)。

## 许可与免责

本项目采用 [MIT 开源许可证](LICENSE)，中文说明与免责声明见该文件；第三方素材与云接口条款见 [NOTICE](NOTICE)。

**免责声明（请务必阅读）：**

- 本软件按「现状」提供，不附带适销性、特定用途适用性等任何担保。
- 问答、诊断、作业单草案等输出**仅供参考**，不构成作业指令、安全结论或合规意见；现场须以现行规程和人工审核为准。
- 本项目为独立开源实现，不代表任何企业或品牌。
- 使用云端模型与第三方接口时，须自行遵守对方服务条款并保管好密钥。
- 作者与贡献者不对因使用本软件而产生的任何直接或间接损失承担责任。
