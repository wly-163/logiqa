...
> **版本:** v1.0 | **日期:** 2026-09-09 | **维护:** LogiQA 架构组
> **输入:** 《N1-N4-incremental-PRD.md》+ 《N1-N4-feature-baseline.md》+ 代码审计
> **范围:** N1 Agent 长期记忆层 / N2 MCP 工具总线 / N3 数字孪生仓储园区 3D / N4 LLM 全链路可观测性
...
### 1.3 N3 数字孪生仓储园区 3D（P1，Week2-3 并行）
...
**设备-空间坐标来源（Q7 确认）：** 首批 华东RDC 园区手工配置坐标模板（~30 台设备），JSON 格式：

```json
{
  "stationId": "rdc-east-demo",
  "stationName": "华东RDC示范仓",
  "areas": [
    {"id": "agv_aisle", "name": "AGV巷道", "position": [0, 0, 0], "size": [12, 0, 8]}
  ],
  "devices": [
    {
      "deviceId": "agv_01",
...
sequenceDiagram
    ...
    U->>AR: 提问 "冷链库A库温超温怎么处理"
    AR->>AMS: recall("冷链库A库温超温", user_id="zhang", scope="user")
    ...
    N-->>AMS: 图记忆1条(偏好:关注AGV类设备)
    AMS-->>AR: "用户偏好：关注AGV类设备；历史诊断：AGV-01曾排除制冷设备故障"
    AR->>AR: messages = [system_prompt, memory_system, user_msg]
    AR->>LLM: chat_with_tools(messages, tools)
    LLM-->>AR: 调用 query_equipment_graph("AGV-01")
    AR->>AR: 工具执行...（多轮ReAct）
    LLM-->>AR: 最终答案
    AR-->>U: 返回答案
    ...
    AMS->>LLM: extract_facts prompt（云模型 1最便宜档）
    LLM-->>AMS: ["用户负责华东RDC城东站","关注AGV-01","已排除制冷设备故障"]
    AMS->>AMS: consolidate（去重/消解/合并）
    AMS->>M: insert memory_collection（新事实向量）
    AMS->>N: MERGE (u)-[:PREFERS]->(e {name:"AGV-01"})
...
sequenceDiagram
    ...
    U->>AR: "雷雨天气下线路SLA违约，查实时遥测"
    AR->>TR: schemas_for(persona.allowed_tools + external_tools)
    TR-->>AR: [search_regulation, query_telemetry, ...]
    AR->>AR: LLM 决定调用 query_telemetry(deviceId="lane_rdc_east_01")
    AR->>TR: run("query_telemetry", {deviceId:"lane_rdc_east_01"})
    TR->>MC: 转发到外部 MCP server
    MC->>MS: MCP call_tool("query_telemetry", {deviceId:"lane_rdc_east_01"})
    MS-->>MC: {temp: -18.2, humidity: 65, agvBattery: 82}
    MC-->>TR: "库温-18.2℃/湿度65%/AGV电量82%"
    TR-->>AR: 工具结果
    AR->>AR: LLM 综合遥测+知识库给出诊断
    AR-->>U: "干线时效偏低，结合暴雨天气判断可能路由中断..."
```
...
sequenceDiagram
    ...
    Note over DT: 页面加载
    DT->>TS: GET /twin/station/rdc-east-demo/layout
    TS-->>DT: {areas, devices[30], connections}
    DT->>TS: GET /twin/station/rdc-east-demo/overview
    ...
    AD->>TS: push_alert_location({severity:"critical", title:"冷链库A库温超温", device:"agv_01"})
    TS->>TS: 查 layout → T1 position=[0, 1.5, 0]
    TS->>WS: broadcast({type:"alert", deviceId:"agv_01", position:[0,1.5,0], severity:"critical"})
    WS-->>DT: {type:"alert", deviceId:"agv_01", ...}
    DT->>DT: 相机飞到 AGV-01 坐标 + 设备红色闪烁
    U->>DT: 点击 AGV-01 设备
    DT->>TS: GET /twin/device/agv_01/detail
    TS->>KG: graph_context("AGV-01", 8)
    TS->>KG: get_paths("AGV-01", depth=3)
    KG-->>TS: 传播链 [AGV-01→过载→末端配送线→订单延误]
...
sequenceDiagram
    ...
    participant AGV as cost_tracker
...
```yaml
  ...
  langfuse:
    ...
    container_name: logiqa-langfuse
  ...
  postgres-langfuse:
    ...
    container_name: logiqa-langfuse-db
    restart: unless-stopped
    environment:
...
def trace_span(name: str, kind=trace.SpanKind.INTERNAL):
    ...
    """
    tracer = trace.get_tracer("logiqa")
...
CATEGORY_PREFERENCE = "preference"    # 用户偏好（如"关注AGV类设备"）
CATEGORY_DIAGNOSIS = "diagnosis"      # 诊断结论（如"排除制冷设备故障"）
...
```python
# warehouse_layout.json 结构（Q7 确认）
{
  "stationId": "rdc-east-demo",
  "stationName": "华东RDC示范仓",
  "warehouseType": "RDC",
  ...
  "areas": [
    {"id": "agv_aisle", "name": "AGV巷道", "position": [0,0,0], "size": [12,0,8]},
    {"id": "outbound_lane_area", "name": "发运月台区", "position": [15,0,0], "size": [10,0,12]},
    {"id": "equipment_room", "name": "设备间", "position": [-12,0,0], "size": [8,4,6]}
  ],
  "devices": [
    {
      "deviceId": "agv_01",
      ...
      "kgEntity": "AGV-01",        # 关联 Neo4j Entity.name（知识图谱/故障链查询用）
...
| Q7 | N3 设备-空间坐标来源 | **确认 PRD 建议：首批 华东RDC 手工配置坐标模板。** 数据结构见 §7.4，存储为 `backend/app/data/warehouse_layout.json`（~30 台设备），twin_service 启动时加载到内存。后续如需配置界面，可新增 `routers/twin.py` 的 PUT /twin/station/{id}/layout 端点 + 前端编辑器（本次不做）。从图谱推断坐标不可靠——Neo4j Entity 无空间属性 |
...
1. **云模型 1 API 可用性**：N1 extract_facts 依赖 云模型 1 API 可用。若 云模型 1 故障，extract_facts 降级跳过（不影响主流程，记忆不更新但不报错）。
2. **Milvus memory_collection 初始化**：在 `main.py` lifespan 中 `ensure_collections()` 时一并创建，与现有 logiqa_chunks 同模式。
