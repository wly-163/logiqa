...
- 容器 pytest：`export MSYS_NO_PATHCONV=1` + `docker cp tests logiqa-backend:/app/tests`（先 rm -rf 防嵌套）+ `docker exec logiqa-backend python -m pytest`
...
docker cp backend/app/models/evidence_gap.py logiqa-backend:/app/app/models/evidence_gap.py
docker exec logiqa-backend python -c "import asyncio;from app.db.base import Base;from app.db.session import engine;from app.models.evidence_gap import EvidenceGap;
async def g():
...
- [ ] **Step 2: 确认失败** `docker exec logiqa-backend python -m pytest tests/test_evidence_gap_service.py -v` → FAIL（模块不存在）
...
```bash
docker cp backend/app/services/evidence_gap_service.py logiqa-backend:/app/app/services/evidence_gap_service.py
docker exec logiqa-backend rm -rf /app/tests && docker cp tests logiqa-backend:/app/tests
docker exec logiqa-backend python -m pytest tests/test_evidence_gap_service.py -v
...
```python
    ...
    if settings.EVIDENCE_GAP_AUTO_COLLEAGV and confidence in ("medium", "refused"):
        try:
