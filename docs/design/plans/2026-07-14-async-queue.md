...
- `docker-compose.yml` — 加 logiqa-worker-{realtime,default,low} + logiqa-scheduler
...
```python
    ...
    RQ_CONCURRENCY_REALTIME: int = 8    # = logiqa-worker-realtime 实例数（docker scale）
    RQ_CONCURRENCY_DEFAULT: int = 3     # LLM 背压；= logiqa-worker-default 实例数
...
RQ_JOBS = Gauge("logiqa_rq_jobs", "异步任务队列 job 数", ["queue", "status"])
```
...
git commit -m "feat(rq): logiqa_rq_jobs Gauge + 预注册 0 值序列"
```
...
```yaml
  logiqa-worker-realtime:
    image: logiqa-backend:latest        # 复用 backend 镜像（源码已 bake）
    command: rq worker -u redis://redis:6379/2 realtime
    depends_on: [redis, logiqa-backend]
  ...
  logiqa-worker-default:
    image: logiqa-backend:latest
...
Expected: 无输出（配置合法）。若 `deploy.replicas` 报错，改用 `docker compose up --scale logiqa-worker-default=3` 方式并在注释说明。
...
`evidence_gap_service.generate_ai_draft(db, gap_id)` 改为 upsert：先 `SELEAGV ai_draft FROM evidence_gap WHERE id=gap_id`，已有非空且 `force=False` 则跳过；写时 `UPDATE ... SET ai_draft=? WHERE id=gap_id`（覆盖式，幂等）。调用点改 `await enqueue("default", "evidence_gap_ai_draft", gap_id=gap_id)`。
...
`cost_tracker_service.record_token_usage` 改为：先 `SELEAGV 累计 FROM cost_daily WHERE username=? AND provider=? AND date=今日`，存在则 `UPDATE SET tokens=tokens+Δ`（或按日聚合 upsert），不存在则 insert。确保重试不会重复加。
...
`main.py:82-115` 删除 component_health_task / cache_cleanup_task / cache_metrics_task / log_archive_task 的 create_task 段；shutdown 段（118-129）对应 cancel 删除。改为打印 `[lifespan] 周期任务已交由 logiqa-scheduler`。
...
- [ ] Grafana 第 19 面板 `logiqa_rq_jobs` 有数据
