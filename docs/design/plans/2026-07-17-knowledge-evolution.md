...
**Tech Stack:** FastAPI + SQLAlchemy(async) + MySQL + Milvus + Redis；embedding_service(qwen)；LLM(deepseek/qwen)；pytest。
...
async def test_draft_create_and_read(test_db):
    d = KnowledgeEvolutionDraft(
        ...
        representative_query="冷链库温超温怎么处理",
        member_queries_json='["冷链库温超温怎么处理"]',
...
def test_cluster_groups_similar():
    items = [
        {"query": "库温超温", "vec": _v([1, 0.01])},
        {"query": "库温报警", "vec": _v([1, 0.02])},   # 与上近似
        {"query": "库温超温", "vec": _v([1, 0.015])},
        {"query": "冷链断链", "vec": _v([0.01, 1])},    # 正交，另成簇
    ]
    clusters = cluster(items, threshold=0.95, min_size=2)
    assert len(clusters) == 1                       # 只库温簇≥2
    assert clusters[0]["representative_query"] in ("库温超温", "库温报警")
...
PROMPT_TMPL = """你是仓储物流知识工程师。基于高频用户疑问和参考资料，编写一条结构化知识条目。
...
async def test_run_scan_persists_drafts(test_db, monkeypatch):
    from app.services import knowledge_evolution_service as ev
    monkeypatch.setattr(ev, "_extract_dislike", lambda *a, **k: _async(["库温超温","库温超温1","库温超温2"]))
    monkeypatch.setattr(ev, "cluster", lambda items, **k: [{"cluster_id":"c1","representative_query":"库温超温","members":[{"query":"库温超温"}]}])
