...
**metrics**：`logiqa_retrieval_tune_total`（扫描次数）/ `logiqa_retrieval_baseline{metric}`（recall/MRR/nDCG 进 Gauge → Grafana 检索质量趋势）。
...
| 云接口欠费致 embed/rerank 失败 | 降级（degraded）+ 报告标注「评测不完整」 |
...
- [ ] `logiqa_retrieval_baseline{metric}` 进 Grafana，检索质量趋势可观测。
