...
系统已有一个**诊断专用 Agentic 服务** `backend/app/services/diagnose_agent_service.py`（设计见 `2026-07-02-agentic-diagnose-design.md`）：LLM 用 function-calling 自主多轮调用工具（检索/图谱/案例/作业单）做交叉验证诊断，返回诊断结果 + 思考链 `steps[]`。其 ReAct 循环、工具注册表、per-tool 异常隔离、降级、指标已相当完整，但 system prompt、工具集、输出格式、降级目标全部**硬编码在诊断域**，无法被其他场景（通用问答、告警处置等）复用。原 spec 早已在 `TOOLS` 注册表处埋下"接口中立、Spec 2/MCP 可直接包装"的泛化意图。
...
- **Provider**：`LLMProvider.chat_with_tools` 已在 deepseek/qwen/doubao 三家实现（均 兼容 chat.completions 协议，均支持 function-calling），返回 `{"content", "tool_calls":[{"id","name","arguments"}]}`。
...
class Tool:
    ...
    @property
    def schema(self) -> dict:   # function-calling schema
...
- `Tool.schema` 输出符合 function-calling 结构。
