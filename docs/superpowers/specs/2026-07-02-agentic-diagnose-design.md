...
> 日期：2026-07-02 ｜ 选型：function-calling agent loop（用户确认）
...
**目标**：新增 `/domain/diagnose-agent`，把"调几次、调哪个工具、何时停"交给 LLM 自主决策（Agentic RAG）。现有快速 diagnose 保留不动，两者并存可对比。诊断过程以 `steps[]` 透出，可解释、可追溯（物流强监管要求）。
...
**为什么 function-calling**：原生结构化（不靠正则）、三家 provider（三路云模型，均 兼容 chat.completions 协议）都支持、零新依赖、自写循环 ~150 行。
...
- **4 个工具**（包装现有 service，不重复造轮子）：
  ...
  | `draft_ticket` | `domain_service.generate_ticket` | 生成处置作业单草案 |
...
- **Provider 扩展**（`providers/base.py` + `llm/{deepseek,qwen,doubao}_llm.py`）
  ...
  - 三家实现：兼容协议 SDK 多传 `tools=` / `tool_choice=`，多取 `message.tool_calls`（每家 ~10 行）。
...
- **指标**（`core/metrics.py`）：复用 `DOMAIN_CALLS.labels("diagnose_agent")` + `DEGRADED.labels("diagnose_agent_*")`；新增 `AGENT_ITERS = Histogram("logiqa_agent_iters", "诊断 agent 循环深度", buckets=(1,2,3,4,5,6,float("inf")))`；`init_metric_series` 预注册 DOMAIN_CALLS label。
...
{"symptom": "冷链库A库温 -12℃ 持续上升，出库负载 70%", "modelType": "deepseek"}
...
# 响应
{
  ...
  "steps": [
    {"iter":1, "thought":"先查规程", "tool":"search_regulation",
     "args":{"query":"AGV库温超温"}, "result":"规程：检查制冷系统/出库负载/库温限值…"},
    {"iter":2, "thought":"查因果链", "tool":"query_equipment_graph",
     "args":{"entity":"AGV-01"}, "result":"风扇故障→过热"},
