...
- 给 **2 个好例 + 1 个坏例**（反噪声）：好例"AGV 故障 → 处置方法 → 绕行作业"；坏例"第二章 → 属于 → 本文"（禁止）。
...
保留现有 11 种，新增 2 个仓储物流高频关系（含别名）：
- **保护**（WMS预警规则 → 被监控设备）：`("保护", "保护范围", "动作于", "SLA违约")`
...
extract_triples(db, doc_id, model_type):
  chunks = SELEAGV chunk WHERE doc_id ORDER BY chunk_idx
  all = []
  for batch(6):
...
- 后处理（集成）：噪声+有效混合输入 → 验证去重、自环过滤、trivial 过滤、归一（"AGV-01"→"AGV"）、关系白名单映射 + 兜底。
- 端到端 mock provider：脚本化 LLM 返回 → 验证最终 normed 三元组集合。
- golden：已知短文本应抽出关键三元组（如"AGV 原因 制冷设备故障"）。
