...
现系统只有**作业单生成**（`backend/app/routers/domain.py:44` `POST /domain/ticket` → `domain_service.generate_ticket`），缺**审核**已填票据合规性的能力。物流安监核心痛点恰是"审"：已填的出库作业单/入库作业单是否缺项、操作顺序是否违安措、危险点是否列出、调度指令编号是否合规。
...
[结构化解析]  →  {ticket_type, 任务, 操作步骤[], 安措[], 危险点[], 调度指令号, ...}
  ...
  │     · sequence：作业顺序违反安措（如"封库隔离"出现在"复核"前）
  │     · danger_point：高风险操作未列危险点（停线/封库/波次切换）
...
- `backend/app/core/metrics.py`：新增 `TICKET_AUDIT = Counter("logiqa_ticket_audit_total", ..., ["result"])`（result: pass/warn/fail）

### 前端
- `frontend/src/views/Diagnose.vue`：新增第 4 个 tab「作业单审核」
  - 输入区：票据类型下拉（出库作业单/入库作业单）+ 大文本框（粘贴票据）+「审核」按钮
...
# 审核报告
{
  ...
  "items": [
    {"layer": "rule|llm", "ruleId": "SEQ_001", "type": "sequence",
     "severity": "critical|major|minor", "msg": "封库隔离出现在复核前",
     "suggestion": "应先复核确认无误后再封库隔离"}
  ],
  "latencyMs": 1234
}
```
