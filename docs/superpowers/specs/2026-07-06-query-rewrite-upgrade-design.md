...
4. **rewrite_query prompt 最 toy**——单段笼统 prompt，无 few-shot、无仓储物流领域示例、无策略分类
...
- **规则**（无 LLM，纯词表 + 长度）：
  ...
  - 缩写：含仓储物流缩写集（WMS/TMS/AGV/RDC/FDC/VMI/IoT…）
...
- `backend/app/data/rewrite_fewshot.json`（仓储物流 few-shot 示例库，按类型组织）
