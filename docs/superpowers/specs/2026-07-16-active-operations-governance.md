...
本次增量把三条能力合成一条可恢复、可审计的闭环：外部实时事件进入统一模型，规则门禁触发只读 Agent，建议经人工确认后只能创建作业单草稿；所有后台工作进入 MySQL 持久任务队列，业务变化写入 Outbox；检索和问答缓存统一执行知识时效硬门禁。

## 主链路

1. `POST /api/realtime/events` 接收 WMS、TMS、IoT 或 generic 事件。
...
5. 有权限的人员确认或驳回建议；确认后可创建 `draft` 作业单。`source_ref=proactive:<run_id>` 保证重复请求不重复建票。
