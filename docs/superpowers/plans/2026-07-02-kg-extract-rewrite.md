...
**Tech Stack:** Python / 兼容协议 SDK（LLM）/ pytest（异步用 `asyncio.run`，无 pytest-asyncio）
...
def test_normalize_dedup_selfloop_trivial_canon():
    raw = [
        {"s": "AGV-01", "r": "原因", "o": "制冷系统故障"},   # 编号前缀去除 → AGV
        {"s": "AGV", "r": "导致", "o": "制冷系统故障"},   # 关系"导致"→"原因"；与上条归一后重复→去重
        {"s": "AGV", "r": "原因", "o": "AGV"},       # 自环→过滤
        {"s": "第二章", "r": "属于", "o": "本文"},             # trivial→过滤
        {"s": "分拣设备", "r": "动作于", "o": "SLA违约"},           # 关系"动作于"→"保护"
    ]
    out = svc._normalize_triples(raw)
    pairs = {(t["s"], t["r"], t["o"]) for t in out}
    assert ("AGV", "原因", "制冷系统故障") in pairs
    assert ("分拣设备", "保护", "SLA违约") in pairs
    assert len(out) == 2
```
...
def test_extract_pipeline_drops_noise_and_canonicalizes():
    ...
    prov = _ScriptedProvider([
        '[{"s":"AGV-01","r":"原因","o":"制冷系统故障"},{"s":"第二章","r":"属于","o":"本文"}]',
        '[{"s":"分拣设备","r":"动作于","o":"SLA违约"},{"s":"AGV","r":"导致","o":"制冷系统故障"}]',
    ])
...
```python
_KG_PROMPT_V2 = """你是仓储物流知识图谱抽取器。从下面运维文本中抽取结构化三元组 (主体, 关系, 客体)。

【实体类型】只抽这些类型的名词短语作为主体/客体：
设备(AGV/叉车/越库月台/立体库/冷链库/危险品库/RDC/FDC等)、部件、异常现象、SLA违约、处置措施、作业步骤、运行参数、危险点、WMS预警规则、标准、系统。

【关系白名单】关系 r 只能从以下选一个；文本中不属于这些语义的【不要抽】：
发生 / 表现为 / 处置方法 / 作业步骤 / 原因 / 影响 / 预防 / 属于 / 位于 / SLA / 预警阈值 / 上游 / 下游
...
【好例】
文本：冷链库温度超过 -15℃阈值，应立即隔离出库并检查冷机。
输出：[{{"s":"冷链库","r":"预警阈值","o":"库温-15℃"}},{{"s":"冷链库","r":"处置方法","o":"隔离出库"}}]
