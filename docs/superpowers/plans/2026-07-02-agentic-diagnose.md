...
**Tech Stack:** FastAPI / Pydantic v2 / 兼容协议 SDK（function-calling）/ Vue 3 / pytest（异步用 `asyncio.run`，无 pytest-asyncio）
...
def test_chat_with_tools_parses_tool_calls(monkeypatch):
    ...
    async def fake_create(**kw):
        ...
        return _make_resp("我先查规程", [("call_1", "search_regulation", '{"query":"AGV库温超温"}')])
...
def test_tool_search_regulation(monkeypatch):
    async def fake_mixed(db, query, topk, **kw):
        return [{"chunk": "库温超 -18℃ 应检查制冷系统与出库负载", "docName": "AGV规程", "score": 0.9}]
...
def test_agent_loop_happy_path(monkeypatch):
    ...
    prov = _ScriptedProvider([
        ("先查规程", [_tc("1", "search_regulation", query="AGV库温超温")]),
        ("再查图谱", [_tc("2", "query_equipment_graph", entity="AGV-01")]),
        ('{"causes":[{"name":"制冷系统故障","likelihood":"高","evidence":"风扇故障","handling":"检查风扇"}],"summary":"冷却不足致过热","risks":["高温SLA违约"]}', None),
    ])
    ...
    r = asyncio.run(svc.diagnose_agent(db=None, symptom="冷链库A库温超温", model_type=None))
...
# ---------- function-calling 工具 schema（Spec 2 MCP 直接复用）----------
TOOLS = [
    {"type": "function", "function": {
        "name": "search_regulation",
        "description": "检索仓储物流规程/手册/标准，获取异常处置的规程依据、限值、标准步骤。",
        "parameters": {"type": "object",
                       "properties": {"query": {"type": "string", "description": "检索关键词，如 'AGV库温超温 处置'"}},
                       "required": ["query"]}}},
    {"type": "function", "function": {
        "name": "query_equipment_graph",
        "description": "查知识图谱中设备的故障-处置因果链（设备→故障→处置 多跳）。",
        "parameters": {"type": "object",
                       "properties": {"entity": {"type": "string", "description": "设备名，如 'AGV-01'"}},
                       "required": ["entity"]}}},
    {"type": "function", "function": {
...
_AGENT_SYSTEM = """你是仓储物流异常诊断专家。基于异常症状，通过调用工具自主收集证据（规程/图谱/历史案例）进行多轮交叉验证后给出诊断。
规则：
...
- Produces: `POST /api/domain/diagnose-agent`（`@limiter.limit("6/minute")` + `get_current_user` + `write_log`）；指标 `logiqa_agent_iters` Histogram + `DOMAIN_CALLS.labels("diagnose_agent")` 预注册
...
async def diagnose_agent(
    ...
    """Agentic 深度诊断：LLM 自主多轮调工具（检索/图谱/案例/作业单）交叉验证，返回诊断 + 思考链 steps。"""
