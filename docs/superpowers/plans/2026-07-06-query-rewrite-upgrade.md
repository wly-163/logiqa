...
- `backend/app/data/rewrite_fewshot.json` — 仓储物流 few-shot 示例库（按类型）
...
def test_abbreviation():
    r = classify("危险品库位设备漏气怎么处理")
    assert r["type"] == "abbreviation"
    assert r["skip"] is False

def test_normal_skipped():
    r = classify("AGV绕组温度过热的应急处置步骤")
...
{
  "colloquial": [
    {"q": "AGV故障咋办", "r": "AGV故障的应急处置流程"}
  ],
  "abbreviation": [
    {"q": "冷链断链", "r": "危险品 分拣设备气体泄漏的检测与处理方法"}
  ],
  "term_missing": [
    {"q": "AGV怎么选", "r": "自动导引车(AGV) 选型原则与技术参数"}
  ]
}
```
...
```bash
docker exec logiqa-backend alembic revision --autogenerate -m "add rewrite_event"
```
...
```bash
docker exec logiqa-backend alembic upgrade head
...
async def test_normal_query_skipped():
    ...
    with patch.object(query_rewrite, "get_llm_provider") as mk_llm:
        r = await query_rewrite.rewrite_query_v2("AGV绕组温度过热的应急处置步骤", None)
        assert r["query"] == "AGV绕组温度过热的应急处置步骤"
...
def _build_prompt(query: str, strategy: dict) -> str:
    ...
    return (
        "你是仓储物流检索查询改写助手。把下面提问改写为更规范、信息更完整、适合向量检索的查询"
...
curl -s -X POST http://localhost:8001/api/qa/answer -H "Authorization: Bearer $TOK" -H "Content-Type: application/json" -d '{"query":"AGV故障咋办","modelType":"deepseek"}'
sleep 2
# 看事件是否记录
...
```bash
docker exec logiqa-backend python /app/scripts/eval_retrieval.py 2>&1 | tail -20
