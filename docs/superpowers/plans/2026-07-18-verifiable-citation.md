...
def test_extract_pdf_structured_has_page_num():
    ...
    buf = io.BytesIO()
    c = canvas.Canvas(buf)
    c.drawString(100, 750, "冷链库温限值-18℃")
    c.showPage(); c.save()
...
def test_extract_xlsx_has_table_header():
    ...
    ws.append(["序号", "设备", "限值"]); ws.append(["1", "AGV", "85"])
...
def test_split_structured_passes_citation_meta():
    ...
    sections = [
        {"type": "text", "content": "冷链库温应不超过-18℃，超过需申请停运。", "page_num": 3, "bbox": "[1,2,3,4]"},
...
def test_build_index_maps_position_to_chunk_id():
    ...
    contexts = [
        {"chunkId": "c1", "chunk": "库温限值", "docName": "A"},
...
def test_citation_answer_schema_parse():
    ...
    raw = {
        "answer_text": "冷链库温应≤-18℃[1]。",
        "citation_map": [{"sentence": "冷链库温应≤-18℃", "ref_id": 1, "chunk_id": "c1",
...
def test_parse_citation_answer_degrades_on_plain_text():
    ...
    ans = parse_citation_answer("冷链库温应≤-18℃[1]。", index={1: "c1"})
    assert ans.answer_text == "冷链库温应≤-18℃[1]。"
    # 降级路径：[1] 反查到 c1
...
```python
SYSTEM_PROMPT = """你是仓储物流专业问答助手，严格依据"参考资料"作答，覆盖生产物流、一盘货统仓统配、送装一体场景。
规则：
1) 只能基于参考资料作答；资料中没有的信息，必须回答"根据现有资料无法确认"，禁止编造。
2) 涉及操作步骤、检修、故障处置时，必须按资料原文顺序，不得遗漏关键步骤。
3) 涉及停线、波次切换、危险品作业等高风险操作，必须在答案末尾追加"⚠ 安全提示：操作前核对调度单与安全规程"。
4) 答案中引用资料时，在对应句末标注 [1]、[2] 等编号，编号对应参考资料序号。
5) 专业术语首次出现时给出全称（如"AGV(自动导引车)"）。
...
def test_verify_claims_three_way(monkeypatch):
    ...
    async def fake_chat(messages, temperature=0, max_tokens=800):
        # 模拟 LLM 返回逐条判定
        return '{"claims":[{"text":"库温限值-18℃","label":"support"},{"text":"无关干扰项","label":"contradict"},{"text":"背景介绍","label":"neutral"}]}'
...
def test_judge_hallucination_still_works_after_extract(monkeypatch):
    ...
    """
    async def fake_chat(messages, temperature=0, max_tokens=800):
        return '{"claims":[{"text":"库温限值-18","supported":true}],"supported_count":1,"total_count":1}'
    monkeypatch.setattr("app.providers.factory.get_llm_provider",
                        lambda mt: type("P", (), {"chat": staticmethod(fake_chat)})())
    from app.rag import judge
    res = _run(judge.judge_hallucination("库温限值-18", ["资料A"], "deepseek"))
...
async def _verify_claims(
    ...
    prompt = (
        "你是仓储物流证据核验员。逐条判断【声明】能否被【参考资料】支撑，输出三分类：\n"
...
def test_verify_nli_contradict_drops(monkeypatch):
    ...
    from app.rag.citation_verifier import verify
    from app.schemas.citation import CitationItem
    cmap = [CitationItem(sentence="无关干扰项", ref_id=1, chunk_id="c1")]
    res = _run(verify("无关干扰项[1]", cmap, {1: "c1"}, [{"chunkId": "c1", "chunk": "核辐射不在保障范围"}], "deepseek",
                      nli_enable=True))
...
[
  {
    ...
    "category": "单句单证据",
    "query": "冷链库温限值是多少",
    "answer": "冷链库温应不超过-18℃[1]。",
    "contexts": [{"chunkId": "c1", "chunk": "冷链运行规定：库温不得超过-18℃。", "docName": "AGV运行规程", "page_num": 12}],
    "expect_refs": [1],
  ...
  },
  {
    "id": "single-multi",
    "category": "单句多证据",
    "query": "危险品库位温湿度异常如何处理",
    "answer": "危险品库温偏离应隔离复核[1]，低于闭锁值禁止启停[2]。",
    "contexts": [
      {"chunkId": "c1", "chunk": "危险品库温偏离阈值应进行隔离复核。", "docName": "危险品维护手册", "page_num": 5},
      {"chunkId": "c2", "chunk": "危险品库温低于闭锁值时严禁进行启停作业。", "docName": "安全规程", "page_num": 22}
    ],
    "expect_refs": [1, 2],
    "expect_support_ratio": 1.0
  },
  {
    ...
    "query": "越库月台可以带载越库吗",
    "answer": "越库月台严禁带载越库[1]。",
    "contexts": [{"chunkId": "c1", "chunk": "越库月台允许带载越库。", "docName": "错误干扰文档", "page_num": 1}],
    "expect_refs": [],
  ...
  },
  {
    ...
    "query": "AGV停机检修时限要求",
    "answer": "AGV停机检修需提前3天申报[1]，未申报不得擅自操作[2]。",
    "contexts": [
      {"chunkId": "c1", "chunk": "AGV停机检修应在操作前3个工作日申报调度。", "docName": "波次拣选规程", "page_num": 8},
      {"chunkId": "c2", "chunk": "未经调度申报严禁擅自进行停线操作。", "docName": "安全规程", "page_num": 15}
    ],
    "expect_refs": [1, 2],
    "expect_support_ratio": 1.0
  }
