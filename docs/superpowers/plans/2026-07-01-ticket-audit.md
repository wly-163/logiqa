...
- **Modify:** `frontend/src/views/Diagnose.vue` — 第 4 个 tab「作业单审核」+ 结果渲染
...
**Interfaces:**
- Produces:
  - `parse_ticket(text: str, ticket_type: str = "作业单") -> dict`，返回结构：
    ```python
...
```python
"""作业单智能审核单测：解析 / 规则引擎 / LLM语义 / 聚合 / 端到端。"""
import asyncio
...
_DEFAULT_RULES = {
    ...
    "sequences": [
        {"id": "SEQ_001", "before": "封库隔离", "after": "复核", "severity": "critical",
         "msg": "封库隔离出现在复核之前", "suggestion": "应先复核确认无误后再封库隔离"},
        {"id": "SEQ_002", "before": "封库", "after": "复核", "severity": "critical",
         "msg": "封库出现在复核之前", "suggestion": "应先复核确认无误后再封库"},
    ],
    "danger_keywords": ["停线", "封库", "波次切换", "越库", "带载", "叉车", "登高", "封库隔离"],
...
```json
{
  ...
  "sequences": [
    {"id": "SEQ_001", "before": "封库隔离", "after": "复核", "severity": "critical", "msg": "封库隔离出现在复核之前", "suggestion": "应先复核确认无误后再封库隔离"},
    {"id": "SEQ_002", "before": "封库", "after": "复核", "severity": "critical", "msg": "封库出现在复核之前", "suggestion": "应先复核确认无误后再封库"}
  ],
  "danger_keywords": ["停线", "封库", "波次切换", "越库", "带载", "叉车", "登高", "封库隔离"],
...
- Produces:
  - `async audit_ticket(text: str, ticket_type: str = "作业单", model_type: str | None = None) -> dict`
    ```python
...
def test_llm_check_parses_items(monkeypatch):
    ...
    items = asyncio.run(svc._llm_check(
        {"task": "t", "steps": ["停线"], "safety": [], "dangers": ["错发错配"]}, "作业单", None))
    assert len(items) == 1
...
```python
_LLM_AUDIT_PROMPT = """你是仓储物流作业单审核专家。规则引擎已查硬性项（必填/顺序/危险点/调度格式/禁用术语），
你专查语义合规：
1. 安措是否覆盖所有危险点与高风险操作
...
- Produces:
  ...
  - 指标 `logiqa_ticket_audit_total{result="pass|warn|fail"}`，启动预注册 0 值
...
async def ticket_audit(
    ...
    """作业单智能审核：已填票据 → 规则引擎+LLM 双层审核报告（仅管理员）。"""
    data = await ticket_audit_service.audit_ticket(body.ticketText, body.ticketType, body.modelType)
    await write_log(db, user.username, "作业单审核", f"{body.ticketType} | {body.ticketText[:40]}")
...
模板：在 `<div class="tabs">` 内「📝 作业单生成」按钮后追加：
```html
      <button class="tab" :class="{ active: tab === 'audit' }" @click="tab = 'audit'">🔍 作业单审核</button>
```
在「作业单生成」`<div class="card" v-show="tab === 'ticket'">` 之后追加第 4 张卡片：
```html
    <!-- 作业单审核 -->
    <div class="card" v-show="tab === 'audit'">
      <div class="row" style="margin-bottom: 14px">
        <select class="select" v-model="auditType" style="max-width:120px"><option value="作业单">作业单</option></select>
        <select class="select" v-model="modelType" style="max-width:140px"><option value="">默认模型</option><option value="deepseek">云模型 1</option><option value="qwen">云模型 2</option><option value="doubao">云模型 3</option></select>
...
```js
const auditText = ref(''); const auditType = ref('作业单'); const auditLoading = ref(false); const audit = ref(null)
...
```
手动：以 admin 登录 → 诊断页 → 「🔍 作业单审核」tab → 选作业单 → 粘贴一张顺序错的票 → 「开始审核」→ 总览 fail + items 列表（severity badge + ruleId + 建议）。普通用户点审核应 toast「审核失败（需管理员权限）」。

- [ ] **Step 4: Commit**
```bash
git add frontend/src/api/index.js frontend/src/views/Diagnose.vue
git commit -m "feat(ticket-audit): 前端作业单审核 tab（Diagnose 第4页 + auditTicket API）"
```
