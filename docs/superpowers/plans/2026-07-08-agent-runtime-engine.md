...
**Tech Stack:** Python 3 / FastAPI / SQLAlchemy async / 兼容 chat.completions 协议 function-calling（deepseek/qwen/doubao 三家已实现 `chat_with_tools`）/ prometheus_client / pytest（**异步用 `asyncio.run`，项目无 pytest-asyncio**）。
...
def build_default_registry() -> ToolRegistry:
    ...
    reg.register(Tool("search_regulation",
                      "检索仓储物流规程/手册/标准，获取异常处置的规程依据、限值、标准步骤。",
...
def test_agent_metrics_preregistered_in_registry():
    ...
    assert "logiqa_agent_calls_total" in text
    assert 'persona="diagnose"' in text
    # AGENT_TOOL_CALLS 4 工具预注册
    assert "logiqa_agent_tool_calls_total" in text
...
Expected: FAIL — `AssertionError`（`logiqa_agent_calls_total` 不在 /metrics 文本）
...
def test_diagnose_fallback_strips_prefix_and_calls_domain(monkeypatch):
    ...
    out = asyncio.run(agent_personas._diagnose_fallback(
        db=None, user_msg="故障症状：冷链库A库温超温", model_type=None))
    assert captured["symptom"] == "冷链库A库温超温"  # 前缀已剥离
...
_DIAGNOSE_SYSTEM = """你是仓储物流异常诊断专家。基于异常症状，通过调用工具自主收集证据（规程/图谱/历史案例）进行多轮交叉验证后给出诊断。
规则：
...
def test_diagnose_agent_migration_returns_stable_schema(monkeypatch):
    ...
    fake = FakeProvider([
        {"content": "查规程", "tool_calls": [
            {"id": "1", "name": "search_regulation", "arguments": {"query": "库温超温"}}]},
        {"content": "查图谱", "tool_calls": [
            {"id": "2", "name": "query_equipment_graph", "arguments": {"entity": "AGV-01"}}]},
