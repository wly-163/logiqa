...
def test_recall_at_k():
    expect = ["仓储运维手册.pdf", "故障案例.docx"]
    got = ["其他.doc", "仓储运维手册.pdf", "故障案例.docx"]
...
def test_ndcg_graded():
    # relevant_docs: {"仓储运维手册.pdf": 3, "其他.doc": 1}
    rd = {"仓储运维手册.pdf": 3, "其他.doc": 1}
    got = ["仓储运维手册.pdf", "其他.doc"]
    score = _ndcg(rd, got)
    assert 0 < score <= 1.0
```
...
- [ ] `logiqa_retrieval_baseline{metric}` 进 Grafana
- [ ] 云接口欠费时扫描降级，报告标注「评测不完整」，不 500
