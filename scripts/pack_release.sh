#!/usr/bin/env bash
# ============================================================================
# 供应链物流 RAG 系统 — 发行包打包脚本
#
# 在开发机(Windows Git Bash 或 Linux)上运行，产出自包含发行包：
#   logiqa-release-<ver>.tar.gz
#
...
#
# 前置: docker compose 全栈已在跑(logiqa-mysql / logiqa-backend 等容器 Up)。
# ============================================================================
set -euo pipefail
...
docker ps --filter "name=logiqa-mysql" --format '{{.Names}}' | grep -q . \
  || { err "logiqa-mysql 容器未运行，请先 docker compose up -d"; exit 1; }
docker ps --filter "name=logiqa-backend" --format '{{.Names}}' | grep -q . \
  || { err "logiqa-backend 容器未运行"; exit 1; }
info "容器就绪"
...
docker exec logiqa-mysql sh -c \
  'exec mysqldump -u"$MYSQL_USER" -p"$MYSQL_PASSWORD" \
     --single-transaction --routines --triggers --default-character-set=utf8mb4 \
     --no-tablespaces "$MYSQL_DATABASE"' \
  > "$REL/docker-entrypoint-initdb.d/logiqa.sql" 2>/dev/null
SQL_LINES=$(wc -l < "$REL/docker-entrypoint-initdb.d/logiqa.sql")
SQL_SIZE=$(du -h "$REL/docker-entrypoint-initdb.d/logiqa.sql" | cut -f1)
grep -q "CREATE TABLE \`documents\`" "$REL/docker-entrypoint-initdb.d/logiqa.sql" \
  || { err "dump 缺少 documents 表，异常"; exit 1; }
info "logiqa.sql: ${SQL_LINES} 行, ${SQL_SIZE}"

# ---- 3. 模型缓存(bge) ----
step "3/6 导出模型缓存 → data/hf-cache"
# 先触发 bge 加载(已缓存则秒返),保证模型落盘
info "预热 bge 模型(若已缓存则秒返)..."
docker exec logiqa-backend python -c \
  "from app.providers.embedding.bge_embedding import _get_model; _get_model()" >/dev/null 2>&1 || \
  warn "bge 预热失败(可能首次未下载),继续导出现有缓存"
docker cp logiqa-backend:/root/.cache "$REL/data/hf-cache" 2>/dev/null
...
echo ""
echo " 分发与使用:"
echo "   1) 把 logiqa-release-$VER.tar.gz 拷给接收方"
echo "   2) 接收方: tar -xzf logiqa-release-$VER.tar.gz"
echo "   3) cd logiqa-release-$VER && cp .env.template .env && (填 Key)"
