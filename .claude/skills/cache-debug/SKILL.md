...
docker exec logiqa-redis redis-cli --scan --pattern "qa:*:{归一化query}"
docker exec logiqa-redis redis-cli GET "qa:default:一盘货库存不足如何跨仓调拨"
```
key 不存在 → 没写过（看步骤 4 写条件）或被 LRU 淘汰/黑名单失效。

### 3. 黑名单拦了吗？
```bash
docker exec logiqa-redis redis-cli SISMEMBER "qa:cache:blacklist" "归一化query"
...
### 6. MySQL L2 有吗？
```bash
docker exec logiqa-mysql mysql -uroot -p<pwd> rag -e \
...
### 7. 降级了吗？
```bash
docker logs logiqa-backend 2>&1 | grep "降级:.*cache"
# 或 Grafana logiqa_degraded_total{tag=~".*cache.*"}
```
