...
curl -s http://localhost:8001/metrics | grep "logiqa_degraded_total" | sort -t' ' -k2 -rn
```
或 Grafana 降级面板（`sum by (tag) (increase(logiqa_degraded_total[5m]))`）。

### 2. 看降级日志（具体原因）
```bash
docker logs logiqa-backend 2>&1 | grep "降级:" | tail -30
...
| tag | 含义 | 根因 |
|---|---|---|
| `rerank` | rerank 调用失败 | 云接口欠费/key 失效/网络；pool 保持原序 |
