...
- logiqa_chunks：云 embedding（EMBEDDING_DIM=1024）
- logiqa_chunks_bge：本地 bge（BGE_DIM，文档小走本地）
...
def _ensure_one(name: str, dim: int) -> None:
    if not utility.has_collection(name):
        ...
        col = Collection(name, CollectionSchema(fields, "供应链物流知识分块"), using="default")
        col.create_index(
            "embedding",
...
def delete_by_doc(doc_id: str) -> None:
    ...
    for name in (settings.MILVUS_COLLECTION, settings.MILVUS_COLLECTION_BGE):
        if utility.has_collection(name):
            col = Collection(name)
            col.load()  # Milvus 2.4：delete 前必须 load，否则 code=101 collection not loaded
...
def ensure_memory_collection() -> None:
    ...
    与 logiqa_chunks 不同的是字段集：pk/embedding/text/user_id/scope/entity/category。
    """
    _connect()
