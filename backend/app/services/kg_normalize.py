...
LLM 自由抽取会产生两类噪声：
- 同义实体分裂："一盘货/统仓统配/一盘货统仓统配" 成多个节点，多跳推理断链
- 关系发散："是/有/关于/涉及" 等无语义关系，图谱 schema 不收敛

收敛策略：
- canonical_entity：去编号前缀（#1/1号/一、）+ 复用 term_service 别名归一
...
# 关系 schema 白名单（供应链物流核心语义）—— 标准: (别名...)
_REL_SCHEMA = {
    "发生": ("发生", "出现", "产生"),
    "表现为": ("表现为", "症状", "现象是", "表现为"),
    "处置方法": ("处置方法", "处理方法", "处置", "处理", "应对", "消除"),
    "作业步骤": ("作业步骤", "操作步骤", "检修步骤", "检修", "处理步骤"),
    "原因": ("原因", "因为", "导致", "引发", "由"),
    "影响": ("影响", "波及", "后果", "危害"),
    "预防": ("预防", "预防措施", "防范", "防止"),
    "属于": ("属于", "归属", "是"),
    "位于": ("位于", "存放在", "位置", "设在"),
    "SLA": ("SLA", "时效", "承诺时效", "服务水平"),
