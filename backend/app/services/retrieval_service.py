...
async def _embed_bge_or_none(text: str) -> list[float] | None:
    """本地 bge 不可用时降级为 None（容器无 HF 缓存时常见），不阻断云 embedding 检索。"""
    try:
        return await embedding_service.embed_query(text, "bge")
    except Exception as e:
        degraded("embed_bge", e)
        return None
...
async def _search_dense_dual(dense_q: str, cand: int, ef: int | None = None) -> tuple[list[dict], list[dict]]:
    ...
    if qvec_bge is not None:
        try:
            dense_bge = await asyncio.to_thread(
                milvus_client.search, settings.MILVUS_COLLECTION_BGE, qvec_bge, cand, _ef
            )
        except Exception as e:
            degraded("milvus_search_bge", e)
            dense_bge = []
    return dense_cloud, dense_bge
...
async def _dense_and_sparse(
    ...
    dense_cloud, dense_bge = await _search_dense_dual(dense_q, cand, _ef)
    dense_hits = []
    ...
    for qq in queries:
        ...
        if getattr(settings, "HYDE_ENABLE", False):
            ...
            try:
                ...
                degraded("hyde_dispatch", e)
        dense_cloud, dense_bge = await _search_dense_dual(dense_q, cand, config_service.rt_ef())
        dense_hits = []
        for src, results in (("dense_cloud", dense_cloud), ("dense_bge", dense_bge)):
            for d in results:
