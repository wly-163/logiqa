...
async def to_ticket(db, disp_id: int, creator: str = "system", tenant: str = "default") -> dict:
    """已确认预案 → 创建作业单草稿（不自动提交审核，留给人工走 submit_for_review）：confirmed → ticketed。"""
    ...
    if row.status != "confirmed":
        raise ValueError("仅已确认(confirmed)预案可转作业单")
    try:
    ...
    ticket = await ticket_lifecycle_service.create_ticket(
        db,
        ticket_type=draft.get("ticket_type") or draft.get("ticketType") or "作业单",
