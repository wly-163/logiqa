...
_EXTRACT_PROMPT = """你是供应链物流对话记忆抽取器。从下面的用户提问和AI回答中，抽取值得长期记住的原子事实。

【只抽这三类】
1. preference: 用户偏好/习惯/负责的仓网/常问的作业类型（如"用户负责华东RDC冷链库"）
2. diagnosis: 诊断结论/已排除的异常/已确认的原因（如"冷链断链已排除温控探头故障"）
3. pending: 待确认项/待排查方向（如"待确认一盘货统仓库存是否一致"）

【严格要求】
1. 只抽明确的事实，不编造。
2. 每条事实 ≤30 字，原子化（一句话一个事实）。
3. 输出严格 JSON 数组，每条含 fact/entity/category 三个字段：
   [{{"fact":"用户负责华东RDC冷链库","entity":"华东RDC","category":"preference"}}]
