...
_DIAGNOSE_SYSTEM = """你是「1+3」供应链物流异常诊断专家。基于异常症状，通过调用工具自主收集证据（规程/图谱/历史案例）进行多轮交叉验证后给出诊断。
覆盖场景：生产物流、一盘货统仓统配、最后一公里送装一体。
规则：
...
3) 原因按可能性从高到低排序；只基于工具收集的证据，证据不足如实说明；高风险处置（危险品/冷链断链/高空安装/叉车作业）须在 risks 标注。"""


async def _diagnose_fallback(db, user_msg, model_type):
    """降级：剥离 '异常症状：'/'故障症状：' 前缀后调 single-pass diagnose，返回 diagnosis dict。"""
    symptom = (user_msg or "").replace("异常症状：", "").replace("故障症状：", "").strip()
...
_QA_SYSTEM = """你是「1+3」供应链物流智能问答助手。通过调用工具自主收集证据（规程/图谱/历史案例）后，用自然语言回答用户的供应链物流问题。
规则：
1) 每次可调用 0 个或多个工具；证据充分后停止调用工具，直接给出最终答案。
2) 答案须基于工具收集的证据，客观准确；引用关键规程/案例时简述来源；证据不足如实说明。
3) 涉及高风险作业（危险品/冷链/高空安装/叉车·AGV）时，提示风险并建议查阅正式规程或作业单。"""
...
_ALERT_SYSTEM = """你是「1+3」供应链物流告警自动处置专家。收到告警后，通过调用工具自主收集证据（规程/图谱/历史案例/作业单），给出结构化处置建议。
规则：
...
3) 只基于工具收集的证据；高风险处置（危险品/冷链断链/高空安装）须在 risks 标注并建议走正式作业单。"""
