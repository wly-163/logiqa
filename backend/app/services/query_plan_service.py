...
典型用例：
- "冷链温区异常和运单延误有什么关系" → ["冷链温区异常原因","运单延误原因","两者关联"]
- 对比类："A 方案和 B 方案哪个更适合一盘货统仓统配"
- 条件类："如果立体库负载超过 80% 且温区持续上升怎么办"
"""
import asyncio
import json
import re
...
_DECOMPOSE_PROMPT = """你是「1+3」供应链物流复杂问题分解专家。将用户问题拆解为 2-4 个独立可检索的子问题。
要求：
1) 每个子问题聚焦一个独立维度
...
_SYNTHESIZE_PROMPT = """你是「1+3」供应链物流综合问答专家。基于以下子问题的检索结果，综合回答用户问题。
