...
def _fmt_ticket(ticket):
    ...
    return (f"节点:{ticket.get('device') or '无'}\n"
...
_SCHEMA_QUERY = {"type": "object",
                 "properties": {"query": {"type": "string", "description": "检索关键词，如 '冷链温区异常 处置'"}},
                 "required": ["query"]}
_SCHEMA_ENTITY = {"type": "object",
                  "properties": {"entity": {"type": "string", "description": "仓配节点名，如 '华东仓1号立体库'"}},
                  "required": ["entity"]}
_SCHEMA_SYMPTOM = {"type": "object",
                   "properties": {"symptom": {"type": "string", "description": "异常症状描述"}},
                   "required": ["symptom"]}
_SCHEMA_TASK = {"type": "object",
                "properties": {"task": {"type": "string", "description": "作业任务，如 '冷链区温区异常处置与换仓'"}},
                "required": ["task"]}
