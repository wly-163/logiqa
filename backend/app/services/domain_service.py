"""领域增强服务：异常诊断推理(D1) / 相似案例检索(D2) / 作业单辅助生成(D3)。

「1+3」供应链物流核心价值功能，复用检索/图谱基建，不重复造轮子：
- diagnose：症状 → 多查询分解 → 并行检索 + 图谱因果 → 可能原因排序 + 处置 + 风险
- similar_case：异常案例库(docType=故障案例)相似检索，"历史上类似异常怎么处理的"
- generate_ticket：作业任务 → 检索规程 → 结构化作业单(步骤/安全措施/风险点)
"""
import json
import re
...
_DIAG_DIM_PROMPT = """你是「1+3」供应链物流诊断专家。根据异常症状，列出 3-5 个需要排查的可能原因方向（每个方向是一句可独立检索的短语，聚焦仓配节点/运输环节/异常类型）。
只输出 JSON 字符串数组，如 ["方向1","方向2"]，不要解释：
症状：{symptom}"""

_DIAG_PROMPT = """你是「1+3」供应链物流异常诊断专家。基于检索资料和知识图谱因果链，对异常症状给出诊断结论。
输出严格 JSON：
{{"causes":[{{"name":"可能原因","likelihood":"高/中/低","evidence":"资料依据","handling":"处置措施"}}],"summary":"总体判断","risks":["风险点1"]}}
要求：原因按可能性从高到低排序；只基于资料，资料不足如实说明；涉及危险品/冷链断链/高空安装/叉车作业等高风险处置须在 risks 标注。
...
_TICKET_PROMPT = """你是「1+3」供应链物流作业单专家。根据作业任务和规程资料，生成结构化作业单。
输出严格 JSON：
{{"device":"涉及节点/设备","steps":["作业步骤1（严格按规程顺序）","作业步骤2"],"safety":["安全措施1"],"risks":["风险点1"],"notes":"备注"}}
要求：步骤严格按规程顺序，不得遗漏关键步骤；涉及危险品/冷链/高空安装必须列安全措施；只基于规程资料，无依据如实说明。
...
async def similar_case(
    ...
    *,
    ...
    """相似历史异常案例检索（D2）：限定案例库 docType=故障案例（兼容既有类型字段）。"""
    _guard(symptom)
    try:
...
async def generate_ticket(
    ...
    *,
    ...
    try:
        ...
        ticket = {"notes": "作业单生成失败，请参考规程资料", "steps": []}
    try:
        from app.core import metrics
