...
# 单行字段（任务 / 调度单号 / 作业人|负责人）—— 顺序无关，re.search 全文找
_FIELD_RE = [
    ("task", r"(?:作业任务|操作任务|工作任务|任务)\s*[:：]\s*(.+)"),
    ("dispatch_no", r"(?:调度)?(?:单|指令|命令)(?:号|编号)?\s*[:：]\s*(.+)"),
    ("operator", r"(?:作业人|操作人|工作负责人|负责人|监护人)\s*[:：]\s*(.+)"),
]

_STEP_MARKERS = ("作业步骤", "操作步骤", "步骤", "操作内容")
...
def parse_ticket(text: str, ticket_type: str = "作业单") -> dict:
    ...
    for line in raw.splitlines():
        ...
        # 跳过已被字段正则消费的行
        if re.match(r"(?:作业任务|操作任务|工作任务|任务)\s*[:：]", s) \
           or re.match(r"(?:调度)?(?:单|指令|命令)", s) \
           or re.match(r"(?:作业人|操作人|工作负责人|负责人|监护人)\s*[:：]", s):
            continue
        item = _strip_num(s)
        if not item:
            continue
...
_DEFAULT_RULES = {
    "required_fields": [
        {"id": "REQ_TASK", "field": "task", "label": "任务", "severity": "critical", "suggestion": "补充作业任务"},
        {"id": "REQ_DISPATCH", "field": "dispatch_no", "label": "调度单号", "severity": "major", "suggestion": "补充调度/运单号"},
        {"id": "REQ_OPERATOR", "field": "operator", "label": "作业人", "severity": "major", "suggestion": "补充作业人/负责人"},
    ],
    "sequences": [
        {"id": "SEQ_001", "before": "发运", "after": "复核", "severity": "critical",
         "msg": "发运出现在复核之前", "suggestion": "应先复核单据与实物一致后再发运"},
        {"id": "SEQ_002", "before": "装车", "after": "复核", "severity": "critical",
         "msg": "装车出现在复核之前", "suggestion": "应先复核SKU/数量/批次后再装车"},
    ],
    "danger_keywords": ["危险品", "冷链", "高空", "叉车", "AGV", "登高", "危化品", "超载"],
    "dispatch_pattern": r"^[A-Za-z0-9\-]{4,}$",
    "blocklist": ["口头指令出库", "无单发货", "约时未确认即配送"],
}
    ...
    for kw in rules.get("blocklist", []):
        if kw in blob:
            items.append(_item("rule", "BLOCK_001", "keyword_blocklist", "major",
                               f"含禁用表述：{kw}", "按安全操作规程规范表述"))
    return items


_LLM_AUDIT_PROMPT = """你是「1+3」供应链物流作业单/运单审核专家。规则引擎已查硬性项（必填/顺序/危险点/调度格式/禁用术语），
你专查语义合规：
1. 安措是否覆盖所有危险点与高风险作业
