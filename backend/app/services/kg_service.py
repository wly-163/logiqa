...
_KG_PROMPT_V2 = """你是「1+3」供应链物流知识图谱抽取器。从下面仓配/物流文本中抽取结构化三元组 (主体, 关系, 客体)。

【实体类型】只抽这些类型的名词短语作为主体/客体：
仓配节点(立体库/分拨中心/前置仓/冷链温区/装卸台等)、设备(AGV/叉车/输送线/分拣机/温控设备等)、部件、异常现象、异常、处置措施、作业步骤、运行参数、危险点、作业单、运单、标准、系统。

【关系白名单】关系 r 只能从以下选一个；文本中不属于这些语义的【不要抽】：
发生 / 表现为 / 处置方法 / 作业步骤 / 原因 / 影响 / 预防 / 属于 / 位于 / SLA / 预警阈值 / 上游 / 下游 / 调拨

【严格要求】
1) 只抽文本中明确出现的事实，绝不编造。
2) 主体 s 与客体 o 必须是具体名词短语（节点名/现象/措施/参数），不得是章节号、"本文""本章"等泛指词。
3) 关系 r 必须在白名单内。
4) 输出严格 JSON 数组：[{{"s":"主体","r":"关系","o":"客体"}}, ...]；无合适三元组输出 []。不要解释、不要 markdown 代码块。

【好例】
文本：冷链温区温度超过 8℃，应立即检查制冷机组并启动应急换仓。华东 RDC 上游是 CDC，干线 SLA 为 24 小时。
输出：[{{"s":"冷链温区","r":"预警阈值","o":"温度8℃"}},{{"s":"冷链温区","r":"处置方法","o":"应急换仓"}},{{"s":"华东RDC","r":"上游","o":"CDC"}},{{"s":"干线运输","r":"SLA","o":"24小时"}}]
