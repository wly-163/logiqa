"""N3 数字孪生仓储园区服务：设备-空间映射 + 状态聚合 + 告警推送 + 异常链。

复用现有服务：
- fault_prediction_service.predict()：riskScore → 设备着色
- kg_service.graph_context() / get_paths()：异常传播链
...
# 默认示范仓 ID
_DEFAULT_STATION = "rdc-east-demo"
...
async def get_station_layout(station_id: str = _DEFAULT_STATION) -> dict:
    """获取站点布局模板（areas + devices + connections）。"""
    return _load_layout(station_id)


async def get_station_overview(station_id: str = _DEFAULT_STATION) -> dict:
    ...
    return {
        ...
        "voltageLevel": layout.get("voltageLevel", layout.get("warehouseType", "")),
        "type": layout.get("type", "warehouse"),
        "areas": areas,
...
async def get_device_detail(device_id: str) -> dict:
    ...
    """
    layout = _load_layout(_DEFAULT_STATION)
    devices = layout.get("devices", [])
...
async def get_fault_chain(device_id: str, depth: int = 3) -> list[dict]:
    """获取设备异常传播链（kg_service.get_paths 封装）。"""
    layout = _load_layout(_DEFAULT_STATION)
    devices = layout.get("devices", [])
    dev = next((d for d in devices if d["deviceId"] == device_id), None)
    if not dev:
