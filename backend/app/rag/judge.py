...
async def _verify_claims(
    ...
    prompt = (
        "你是「1+3」供应链物流证据核验员。逐条判断【声明】能否被【参考资料】支撑，输出三分类：\n"
...
async def judge_hallucination(
    ...
    refs = "\n".join(f"[{i + 1}] {s}" for i, s in enumerate(srcs))
    prompt = (
        "你是「1+3」供应链物流问答忠实度核查员。按步骤判断【答案】是否被【参考资料】支撑"
...
async def judge_context_relevance(
    ...
    prompt = (
        "你是「1+3」供应链物流检索质量审核员。判断以下【检索到的文档分块】是否与【用户问题】相关。\n"
...
async def judge_answerability(
    ...
    prompt = (
        "你是「1+3」供应链物流知识审核员。判断仅凭以下【检索到的参考资料】，能否完整回答【用户问题】。\n\n"
        f"【用户问题】\n{query}\n\n"
        f"【参考资料】\n{refs}\n\n"
        "输出 JSON：{\n"
