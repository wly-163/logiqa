"""领域增强接口：故障诊断推理(D1) / 相似历史案例(D2) / 作业单辅助生成(D3)。"""
...
async def ticket(
    ...
    """作业单辅助生成：操作任务→结构化作业单（步骤/安全措施/风险点）。"""
    data = await domain_service.generate_ticket(db, body.task, body.modelType)
    await write_log(db, user.username, "作业单生成", f"任务：{body.task[:40]}")
...
async def ticket_create(
    ...
    await write_log(db, user.username, "创建作业单", f"{body.ticketType}：{body.task[:40]}")
...
async def ticket_submit(
    ...
    await write_log(db, user.username, "作业单提交审核", f"票据{ticket_id}")
...
async def ticket_review(
    ...
    await write_log(db, user.username, "作业单审核", f"票据{ticket_id} {'通过' if body.approved else '驳回'}")
    return success(data, "审核完成")
...
async def ticket_issue(
    ...
    await write_log(db, user.username, "作业单签发", f"票据{ticket_id}")
...
async def ticket_archive(
    ...
    await write_log(db, user.username, "作业单归档", f"票据{ticket_id}")
