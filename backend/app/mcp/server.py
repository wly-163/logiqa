...
4. draft_ticket — 生成处置作业单草案（role=admin 权限）
...
MCP_TOOLS: list[dict] = [
    {
        ...
        "description": "检索供应链物流规程/手册/标准，获取异常处置的规程依据、限值、标准步骤。",
        "inputSchema": {
            "type": "object",
            "properties": {"query": {"type": "string", "description": "检索关键词，如 '冷链断链 处置'"}},
            "required": ["query"],
        },
    },
    {
        ...
        "inputSchema": {
            "type": "object",
            "properties": {"entity": {"type": "string", "description": "设备/库区名，如 '冷链库A'"}},
            "required": ["entity"],
        },
    },
    {
    ...
    {
        "name": "draft_ticket",
        "description": "生成处置作业单草案（步骤/安措/风险）。需要 admin 权限。",
        "inputSchema": {
            "type": "object",
            "properties": {"task": {"type": "string", "description": "操作任务，如 '危品出库复核并装车'"}},
            "required": ["task"],
        },
    },
    {
...
async def call_tool(
    ...
    Body: {"name": "search_regulation", "arguments": {"query": "冷链断链"}}
    """
    _auth(request, authorization)
