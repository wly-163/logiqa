#!/usr/bin/env bash
# ============================================================================
# 供应链物流 RAG 系统 — 接收方 Linux 一键引导脚本
#
# 在发行包解压后的根目录运行。子命令:
#   ./install.sh up         构建并启动全部服务(首次会自动导入 logiqa.sql)
#   ./install.sh stop       停止
...
# ---------- 等待 mysql 健康 ----------
wait_mysql() {
  step "等待 MySQL 就绪(首次启动会自动导入 logiqa.sql,约 30-90s)..."
  local i
  for i in $(seq 1 60); do
    if [ "$(docker inspect logiqa-mysql --format '{{.State.Health.Status}}' 2>/dev/null)" = "healthy" ]; then
      info "MySQL healthy"
      # 校验 documents 表已导入
      if docker exec logiqa-mysql sh -c 'mysql -uroot -p"$MYSQL_ROOT_PASSWORD" logiqa -N -e "SELECT COUNT(*) FROM documents"' 2>/dev/null | grep -qE '[0-9]+'; then
        local cnt; cnt=$(docker exec logiqa-mysql sh -c 'mysql -uroot -p"$MYSQL_ROOT_PASSWORD" logiqa -N -e "SELECT COUNT(*) FROM documents"' 2>/dev/null)
...
wait_backend() {
  ...
  warn "backend 120s 内未响应 /health;执行 docker logs logiqa-backend --tail 30 排查"
}
...
do_reset()   {
  warn "将停止服务并清空 data/mysql(下次 up 重新从 logiqa.sql 导入)。数据卷 neo4j/milvus 等保留。"
  read -r -p "确认? [y/N] " ans
