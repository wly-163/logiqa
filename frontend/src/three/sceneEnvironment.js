...
export function buildCityBlock(config = {}) {
  ...
  const candidates = [
    ...
    // 园区东南(彩钢仓)
    { type: 'warehouse', x: 26, z: -20, w: 5, d: 4, h: 3 },
    { type: 'warehouse', x: 26, z: -25, w: 5, d: 4, h: 3 },
...
function shedWallMat(color) {
  return getMaterial(`shed-wall-${color}`, () =>
    new THREE.MeshStandardMaterial({ color, metalness: 0.32, roughness: 0.58 })
  )
}
...
/** 彩钢仓库：平顶 + 月台卷帘门 */
function buildShed(w, h, d, wallColor, { doors = 0, doorFace = 'z+' } = {}) {
  const g = new THREE.Group()
  const wall = shedWallMat(wallColor)
  const roofC = getMaterial('shed-roof', () =>
    new THREE.MeshStandardMaterial({ color: 0x4A5560, metalness: 0.4, roughness: 0.5 })
  )
  const body = markShadows(new THREE.Mesh(new RoundedBoxGeometry(w, h * 0.88, d, 2, 0.12), wall))
  body.position.y = h * 0.44
  g.add(body)
  const roof = markShadows(new THREE.Mesh(new THREE.BoxGeometry(w * 1.08, 0.18, d * 1.08), roofC))
  roof.position.y = h * 0.92
  g.add(roof)
  if (doors > 0) {
    const doorMat = getMaterial('dock-door', () =>
      new THREE.MeshStandardMaterial({ color: 0x2E4053, metalness: 0.3, roughness: 0.6 })
    )
    const stripe = getMaterial('dock-stripe', () =>
      new THREE.MeshStandardMaterial({ color: 0xF4D03F, metalness: 0.2, roughness: 0.5 })
    )
    const axisX = doorFace.startsWith('x')
    for (let i = 0; i < doors; i++) {
      const t = (i + 0.5) / doors - 0.5
      const dw = Math.min(2.2, (axisX ? d : w) / doors * 0.72)
      const door = new THREE.Mesh(
        axisX ? new THREE.BoxGeometry(0.08, h * 0.55, dw) : new THREE.BoxGeometry(dw, h * 0.55, 0.08),
        doorMat
      )
      const bump = new THREE.Mesh(
        axisX ? new THREE.BoxGeometry(0.1, 0.12, dw) : new THREE.BoxGeometry(dw, 0.12, 0.1),
        stripe
      )
      const along = t * (axisX ? d : w) * 0.82
      if (doorFace === 'z+') {
        door.position.set(along, h * 0.35, d / 2 + 0.04)
        bump.position.set(along, 0.18, d / 2 + 0.08)
      } else if (doorFace === 'z-') {
        door.position.set(along, h * 0.35, -d / 2 - 0.04)
        bump.position.set(along, 0.18, -d / 2 - 0.08)
      } else if (doorFace === 'x+') {
        door.position.set(w / 2 + 0.04, h * 0.35, along)
        bump.position.set(w / 2 + 0.08, 0.18, along)
      } else {
        door.position.set(-w / 2 - 0.04, h * 0.35, along)
        bump.position.set(-w / 2 - 0.08, 0.18, along)
      }
...
function buildIsoContainer(color) {
  const g = new THREE.Group()
  const body = markShadows(new THREE.Mesh(
    new THREE.BoxGeometry(2.4, 1.15, 6.0),
    getMaterial(`ctr-${color}`, () =>
      new THREE.MeshStandardMaterial({ color, metalness: 0.45, roughness: 0.4 })
    )
  ))
  body.position.y = 0.575
  g.add(body)
  const ribs = getMaterial('ctr-rib', () =>
    new THREE.MeshStandardMaterial({ color: 0x1C2833, metalness: 0.3, roughness: 0.6 })
  )
  for (let i = 0; i < 5; i++) {
    const rib = new THREE.Mesh(new THREE.BoxGeometry(2.45, 1.0, 0.08), ribs)
    rib.position.set(0, 0.55, -2.4 + i * 1.2)
    g.add(rib)
  }
  return g
}
...
function buildBoxTruck() {
  const g = new THREE.Group()
  const cab = markShadows(new THREE.Mesh(
    new RoundedBoxGeometry(2.2, 1.5, 1.6, 2, 0.08),
    getMaterial('truck-cab', () =>
      new THREE.MeshStandardMaterial({ color: 0x1A5276, metalness: 0.35, roughness: 0.4 })
    )
  ))
  cab.position.set(0, 1.15, 2.3)
  g.add(cab)
  const cargo = markShadows(new THREE.Mesh(
    new THREE.BoxGeometry(2.4, 2.2, 4.4),
    getMaterial('truck-box', () =>
      new THREE.MeshStandardMaterial({ color: 0xD5D8DC, metalness: 0.25, roughness: 0.55 })
    )
  ))
  cargo.position.set(0, 1.5, -0.4)
  g.add(cargo)
  const wheelM = getMaterial('truck-wheel', () =>
    new THREE.MeshStandardMaterial({ color: 0x1C1C1C, roughness: 0.8 })
  )
  for (const z of [-1.6, 0.4, 2.1]) {
    for (const x of [-1.15, 1.15]) {
      const wh = new THREE.Mesh(new THREE.CylinderGeometry(0.38, 0.38, 0.28, 12), wheelM)
...
/** 仓储中心外围：长条仓、月台门、集装箱堆、货车，不含烟囱厂房 */
export function buildLogisticsPark() {
  const g = new THREE.Group()
  g.name = 'LogisticsPark'

  const south = buildShed(22, 3.2, 7, 0xB0B7BE, { doors: 6, doorFace: 'z-' })
  south.position.set(0, 0, 24)
  g.add(south)

  const north = buildShed(18, 3.0, 6.5, 0xC0392B, { doors: 4, doorFace: 'z+' })
  north.position.set(-4, 0, -24)
  g.add(north)

  const east = buildShed(6.5, 3.4, 20, 0x7F8C8D, { doors: 5, doorFace: 'x-' })
  east.position.set(28, 0, -2)
  g.add(east)

  const west = buildShed(7, 3.6, 14, 0xAED6F1, { doors: 3, doorFace: 'x+' })
  west.position.set(-28, 0, 2)
  g.add(west)

  const office = buildShed(5, 4.2, 5, 0x85929E, { doors: 1, doorFace: 'z+' })
  office.position.set(24, 0, 22)
  g.add(office)

  const sw = buildShed(8, 2.8, 8, 0xA04000, { doors: 2, doorFace: 'z+' })
  sw.position.set(-24, 0, -22)
  g.add(sw)

  const colors = [0x1A5276, 0x922B21, 0x1E8449, 0xB9770E, 0x1B4F72]
  let idx = 0
  for (let row = 0; row < 3; row++) {
    for (let col = 0; col < 4; col++) {
      const c = buildIsoContainer(colors[idx++ % colors.length])
      c.scale.set(0.55, 0.55, 0.55)
      c.position.set(22 + col * 1.5, row * 0.64, -18)
  ...
  for (let i = 0; i < 4; i++) {
    const t = buildBoxTruck()
    t.scale.set(0.55, 0.55, 0.55)
    t.position.set(-10 + i * 7, 0, 16.5)
...
export function buildDistrict(envMap) {
  ...
  // 外围仓配园区（长条仓 / 月台 / 集装箱 / 货车）
  g.add(buildLogisticsPark())
