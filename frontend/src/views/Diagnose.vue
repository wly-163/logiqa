<template>
  <div>
    <div class="tabs">
      <button class="tab" :class="{ active: tab === 'diagnose' }" @click="tab = 'diagnose'">🩺 异常诊断推理</button>
      ...
      <button class="tab" :class="{ active: tab === 'ticket' }" @click="tab = 'ticket'">📝 作业单生成</button>
    ...
    <div class="card" v-show="tab === 'diagnose'">
      <div class="row" style="margin-bottom: 14px">
        <input class="input" v-model="symptom" placeholder="描述异常症状，如：华东 RDC 爆仓，出库波次积压超 4 小时，干线延误率 18%" @keyup.enter="doDiagnose" style="flex:1;min-width:260px" />
    ...
    <div class="card" v-show="tab === 'ticket'">
      <div class="row" style="margin-bottom: 14px">
        <input class="input" v-model="ticketTask" placeholder="作业任务，如：华东 RDC 紧急跨仓调拨补货至华南 FDC" @keyup.enter="doTicket" style="flex:1;min-width:260px" />
        <select class="select" v-model="modelType" style="max-width:140px"><option value="">默认模型</option><option value="deepseek">DeepSeek</option><option value="qwen">通义千问</option><option value="doubao">豆包</option></select>
        <button class="btn btn-primary" @click="doTicket" :disabled="ticketLoading || !ticketTask.trim()">{{ ticketLoading ? '生成中…' : '生成作业单' }}</button>
      </div>
      <div v-if="ticket">
        <p class="warning-tip">⚠ 辅助生成草案，必须由责任人核对调度单号与安全操作规程后才能执行。</p>
        <div class="ticket" v-if="ticket.ticket">
          <div class="t-row" v-if="ticket.ticket.device"><b>涉及节点：</b>{{ ticket.ticket.device }}</div>
          <div class="t-block" v-if="ticket.ticket.steps?.length"><b>作业步骤：</b><ol><li v-for="(s, i) in ticket.ticket.steps" :key="i">{{ s }}</li></ol></div>
    ...
    <div class="card" v-show="tab === 'audit'">
      <div class="row" style="margin-bottom: 14px">
        <select class="select" v-model="auditType" style="max-width:120px"><option value="作业单">作业单</option><option value="运单">运单</option></select>
        <select class="select" v-model="modelType" style="max-width:140px"><option value="">默认模型</option><option value="deepseek">DeepSeek</option><option value="qwen">通义千问</option><option value="doubao">豆包</option></select>
        <button class="btn btn-primary" @click="doAudit" :disabled="auditLoading || !auditText.trim()">{{ auditLoading ? '审核中…' : '开始审核' }}</button>
      </div>
      <textarea class="input" v-model="auditText" placeholder="粘贴已填单据全文（含 任务/调度单号/作业人/作业步骤/安全措施/危险点）" style="width:100%;min-height:160px;resize:vertical;font-family:inherit;margin-bottom:12px"></textarea>
      <div v-if="audit">
