<template>
  <div>
    <div class="stat-grid ops-stats">
      <div class="stat stat-accent"><div class="stat-val">{{ runs.total || 0 }}</div><div class="stat-lbl">主动预警记录</div></div>
    ...
    <div class="card" v-show="tab === 'runs'">
      <div class="card-header">
        <div><h3 class="card-title">主动预警建议</h3><div class="card-desc">Agent 仅做只读分析；确认后也只会生成作业单草稿。</div></div>
          ...
          <tbody>
            <tr v-for="r in runs.list" :key="r.id">
              ...
              <td class="actions">
                ...
                <button v-if="can('alert:manage') && r.status==='confirmed'" class="btn btn-success btn-sm" @click="toTicket(r)">转作业单草稿</button>
            ...
            <tr v-if="!runs.list?.length"><td colspan="7" class="empty">暂无主动预警记录</td></tr>
          </tbody>
        </table>
      </div>
    </div>
    ...
    <div class="card" v-show="tab === 'events'">
      <div class="card-header"><div><h3 class="card-title">统一实时事件流</h3><div class="card-desc">WMS / TMS / OMS / IoT / generic，按租户和源事件 ID 幂等接收。</div></div><button class="btn btn-ghost btn-sm" @click="loadEvents">刷新</button></div>
      <div class="table-wrap">
        <table class="tbl">
...
function statusLabel(s) { return ({ proposed:'待确认', confirmed:'已确认', ticketed:'已转作业单', succeeded:'成功', completed:'完成', published:'已投递', failed:'失败', dead:'死信', running:'运行中', processing:'处理中', dispatching:'投递中', queued:'排队中', pending:'待投递', ignored:'已忽略', rejected:'已驳回' })[s] || s }
function severityBadge(s) { return s === 'critical' || s === 'major' ? 'badge-danger' : s === 'warning' ? 'badge-warning' : 'badge-info' }

async function loadRuns() { try { runs.value = unwrapBiz(await getProactiveRuns({ size: 50 })) || runs.value } catch { toast('主动预警记录加载失败') } }
...
async function toTicket(r) { if (!confirm('仅创建作业单草稿，仍需后续审核、签发和执行。继续？')) return; try { unwrapBiz(await proactiveRunToTicket(r.id)); toast('作业单草稿已创建'); await loadRuns() } catch { toast('创建作业单草稿失败') } }
