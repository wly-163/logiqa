  ...
  <div>
    ...
    <div class="card" v-show="tab === 'persona'">
      ...
      <p class="hint" style="margin-top:0;line-height:1.7">
        ...
        可调：① <b>角色指令</b>——告诉 AI 怎么回答（如「回答简洁、突出安全风险」）；② <b>可用工具</b>——它能查哪些资料（运维规程/知识图谱/历史案例/作业单）；③ <b>参数</b>——思考几轮、回答风格、输出格式。<br/>
      ...
      <textarea class="input edit-area" v-model="personaForm.systemPrompt" placeholder="角色指令：告诉这个 AI 助手它是谁、怎么回答。例如「你是供应链物流专家，回答要简洁专业，突出安全风险，分点说明」（留空 = 用内置默认指令）" rows="3" style="margin:6px 0"></textarea>
    ...
    <div class="card" v-show="tab === 'alert'">
      ...
      <div class="disposal-section" style="margin-top:18px;border-top:1px dashed var(--border);padding-top:12px">
        ...
        <p class="hint" style="margin:0 0 10px">手动触发或 Grafana 告警进来后，AI 自动调工具(规程/图谱/案例/作业单)分析 → 生成诊断/处置/作业单草案。</p>
        <template v-if="can('alert:manage')">
          <div class="disp-form" style="display:flex;gap:6px;align-items:center;flex-wrap:wrap">
            ...
            <input class="input" v-model="dispForm.title" placeholder="告警标题（如：RDC爆仓波次积压）" style="flex:1;min-width:160px" />
        ...
        <div style="overflow-x:auto;margin-top:8px">
          <table class="tbl">
            <thead><tr><th>状态</th><th>告警</th><th>处置概述</th><th>作业单</th><th>来源</th><th>时间</th><th v-if="can('alert:manage')">操作</th></tr></thead>
            <tbody>
              <tr v-for="d in disposals.list" :key="d.id">
                ...
                <td style="white-space:nowrap" v-if="can('alert:manage')">
                  ...
                  <button v-if="d.status==='confirmed'" class="btn btn-accent btn-sm" @click="doToTicket(d)">转作业单</button>
    ...
    <div v-show="tab === 'config'">
      ...
      <div class="card" v-show="tab === 'terms'">
        ...
        <div class="row" style="gap:8px;margin:8px 0">
          <input class="input" v-model="termForm.alias" placeholder="别名/错别字（如：一盘货）" style="flex:1" />
          <input class="input" v-model="termForm.standard" placeholder="标准词（如：一盘货统仓统配）" style="flex:1" />
      ...
      <div class="card" v-show="tab === 'prompt'">
        ...
        <p class="hint" style="margin-top:0">供应链物流问答的 system prompt。改后<b>即改即生效</b>（下次问答即用，热读缓存）。空保存=恢复 code 默认。</p>
      ...
      <div class="card" v-show="tab === 'srules'">
        ...
        <p class="hint" style="margin-top:0">自定义「维度→关键词→标签」规则，给文档/分块文本打语义维度（如 作业场景=跨仓调拨、安全等级=高风险）。</p>
        <div class="row" style="gap:8px;margin:8px 0;flex-wrap:wrap">
          ...
          <input class="input" v-model="srForm.tag" placeholder="标签（跨仓调拨/高风险）" style="width:150px" />
...
function dispStatusLabel(s) {
  return { pending: '⏳处理中', proposed: '🟡待确认', confirmed: '✅已确认', rejected: '❌已驳回', ticketed: '🎫已转作业单', closed: '⚪已关闭', disposed: '✅已处置' }[s] || s
}
...
async function doToTicket(d) { try { await disposalToTicket(d.id); toast('已转作业单草稿，请到作业单管理提交审核'); loadDisposals() } catch (e) { toast('操作失败') } }
