  ...
  <div>
    <div class="card" style="margin-bottom:12px">
      <div class="row" style="gap:8px;align-items:center;flex-wrap:wrap">
        ...
        <span class="muted" style="font-size:12px">· 仓配园区模式</span>
        ...
        <button class="btn btn-ghost btn-sm" @click="viewStation">⚡ 看仓配网络</button>
    ...
    <div class="twin-container" ref="containerRef">
      ...
      <div class="twin-info" v-if="overview.devices?.length">
        ...
        <div>📐 视距 {{ cameraDistance.toFixed(1) }}m · 设备 {{ overview.deviceCount }} · 仓配园区（月台 / 仓库 / 集装箱）</div>
      </div>
    </div>
    ...
    <div class="card" v-if="selectedDevice" style="margin-top:12px">
      <div class="card-header">
        ...
        <button class="btn btn-ghost btn-sm" @click="openKg3d">在3D图谱中查看</button>
...
async function loadOverview() {
  try {
    const r = await getStationOverview('rdc-east-demo')
...
function focusFromRoute() {
  const q = String(route.query.device || route.query.entity || '').trim()
  if (!q) return
  const devices = overview.value.devices || []
  const hit = devices.find((d) => d.deviceId === q || d.kgEntity === q || d.name === q)
    || devices.find((d) => (d.kgEntity && (String(d.kgEntity).includes(q) || q.includes(d.kgEntity)))
      || (d.name && String(d.name).includes(q)))
  if (hit) selectDevice(hit.deviceId)
}
function openKg3d() {
  const d = selectedDevice.value
  if (!d) return
  router.push({ path: '/kg-3d', query: { entity: d.kgEntity || d.name || d.deviceId } })
}
...
function render3D() {
  ...
  try {
    ...
    for (const dev of devices) {
      ...
      root.userData = { deviceId: dev.deviceId, name: dev.name, kgEntity: dev.kgEntity, originalColor: riskColor, riskColor }
      // 给设备材质应用 envMap
      if (envMap) {
        root.traverse(o => {
    ...
    if (showWires.value) {
      ...
      for (const dev of devices) {
        ...
        for (const connId of dev.connections) {
          ...
          // 起止点抬高到设备顶部以上（避开货架/立柱几何，避免线穿过设备"闪"）
