<template>
  <div>
    <div class="tabs">
      <button class="tab" :class="{ active: tab === 'tickets' }" @click="loadTickets(); tab = 'tickets'">📋 单据列表</button>
      <button class="tab" :class="{ active: tab === 'create' }" @click="tab = 'create'">✏️ 新建作业单</button>
    ...
    <div class="card" v-show="tab === 'stats'">
      ...
      <div v-else class="hint" style="margin-top:8px">暂无单据数据</div>
    </div>

    <!-- 新建作业单 -->
    <div class="card" v-show="tab === 'create'">
      <div class="row" style="gap:8px;margin-bottom:10px">
        <label><input type="radio" v-model="form.type" value="作业单" /> 作业单</label>
        <label><input type="radio" v-model="form.type" value="运单" /> 运单</label>
      </div>
      <div class="field"><label class="field-label">作业任务 *</label><input class="input" v-model="form.task" placeholder="如：华东 RDC 紧急跨仓调拨补货至华南 FDC" /></div>
      <div class="field"><label class="field-label">涉及节点</label><input class="input" v-model="form.device" placeholder="如：华东 RDC / 干线车队 A" /></div>
      <div class="field"><label class="field-label">作业地点</label><input class="input" v-model="form.location" placeholder="如：华东区域配送中心" /></div>
      <div class="field"><label class="field-label">作业步骤（每行一步）</label><textarea class="input" v-model="form.stepsText" rows="4" style="width:100%;resize:vertical;font-family:inherit" placeholder="1. 确认库存与调拨单&#10;2. 复核拣货&#10;3. ..."></textarea></div>
      <div class="field"><label class="field-label">安全措施（每行一条）</label><textarea class="input" v-model="form.safetyText" rows="3" style="width:100%;resize:vertical;font-family:inherit" placeholder="1. 双人复核高价值件&#10;2. ..."></textarea></div>
      <div class="field"><label class="field-label">风险点（每行一条）</label><textarea class="input" v-model="form.risksText" rows="2" style="width:100%;resize:vertical;font-family:inherit" placeholder="如：错发错配&#10;冷链断链"></textarea></div>
    ...
    <div class="card" v-show="tab === 'tickets'">
      <div class="card-header">
        <h3 class="card-title">单据列表 <span class="badge badge-neutral">{{ ticketList.total }}</span></h3>
        <div class="row">
      ...
      <div v-if="!ticketList.list?.length" class="empty">暂无单据</div>
...
const can = (p) => hasPerm(auth.role, p)   // RBAC：作业单审核(system:config)仅管理员
...
// 新建
const form = ref({ type: '作业单', task: '', device: '', location: '', stepsText: '', safetyText: '', risksText: '', notes: '' })
